...
// PackAccessStrategy vtables for isTrusted
{
	...
	void* povChange = reinterpret_cast<void*>(FindSignature("48 89 5C 24 ? 57 48 83 EC ? 48 8B 79 ? 48 8B D9 48 83 BF"));
	g_Hooks.onPOVChangeHook = std::make_unique<FuncHook>(povChange, Hooks::onPOVChange);
...
bool Hooks::playerCallBack(C_Player* lp, __int64 a2, __int64 a3) {
	...
	if (g_Data.getLocalPlayer() != nullptr && lp == g_Data.getLocalPlayer()) {
		...
		for (const auto& ent : g_Hooks.entityList) {
			...
			if (entity->isAlive() && entity != nullptr && (__int64)entity != 0x163E88FE850 && (__int64)entity != 0x1FBBF1FE850 && (__int64)entity != 0xFFFFFFFFFFFFFCD7 && ent.ent != nullptr && *(__int64*)ent.ent != 0xFFFFFFFFFFFFFCD7 && *(__int64*)ent.ent > 0x6FF000000000 && *(__int64*)ent.ent < 0x800000000000 && *((int64_t*)ent.ent + 1) < 0x6FF000000000 && *(__int64*)ent.ent <= Utils::getBase() + 0x10000000) validEntities.push_back(ent);
		}
		g_Hooks.entityList.clear();
...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour Kek.Club+ prefix is: \"%s%c%s\"", GREEN, YELLOW, cmdMgr->prefix, RED);
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sEnter \"%s%cprefix .%s\" to reset your prefix", GREEN, YELLOW, cmdMgr->prefix, RED);
			}
		}
	}
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	DrawUtils::setCtx(renderCtx, dat);
	{
		...
		static auto textCustomizerMod = moduleMgr->getModule<Theme>();
		DrawUtils::flush();
		...
		bool shouldRenderWatermark = false;
...

		{
			...
			if (strcmp(screenName.c_str(), "start_screen") == 0) {
				// Draw BIG epic DaDrip watermark

				{
					std::string text = "KEK.CLUB+";
					...
					DrawUtils::drawText(textPos, &text, MC_Color(200, 200, 200, 1), 8.f);
				}

				// Draw Custom Buttons
				if (g_Data.allowWIPFeatures()) {
					// if (HImGui.Button("Custom Button", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
					// }
				}
			} else {
...

				{
					...
					if (shouldRenderWatermark) {
						...
						static std::string name = "Kek.Club+ 0.1.0";
#ifdef _DEBUG
						...
						//DrawUtils::drawRectangle(rect, MC_Color(147, 112, 219), 1.f, 2.f);
				...
				*/
					// Draw ArrayList
					if (moduleMgr->isInitialized() && shouldRenderArrayList && !moduleMgr->getModule<ClickGuiMod>()->isEnabled()) {
						// Parameters
						float textSize = arrayMod->scale;
						float textPadding = 1.f * textSize;
						float textHeight = 10.5f * textSize;
						float smoothness = 2;
						if (!arrayMod->top)
						...
						struct IModuleContainer {
							...
							IModuleContainer(std::shared_ptr<IModule> mod) {
								...
								const char* modNameChr = mod->getModName();
								...
								if (arrayMod->keybinds == 0x0)
									moduleName = moduleNameChr;
								else {
									char text[50];
									sprintf_s(text, 50, "%s", moduleNameChr);
									moduleName = text;
								}
								...
								this->textWidth2 = DrawUtils::getTextWidth(&modName, arrayMod->scale);
							}
							...
							bool operator<(const IModuleContainer& other) const {
								...
								if (this->textWidth + this->textWidth2 == other.textWidth + other.textWidth2)
									return moduleName < other.moduleName;
								return this->textWidth + this->textWidth2 > other.textWidth + other.textWidth2;
							}
						};
						...
						for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
							...
							if (arrayMod->rainBow.selected == 0) {
								if (arrayMod->rainBowDirection.selected == 1)
									arrayColor = DrawUtils::getWaveRGB(arrayMod->r1, arrayMod->g1, arrayMod->b1, arrayMod->r2, arrayMod->g2, arrayMod->b2, -index * arrayMod->sped);
								if (arrayMod->rainBowDirection.selected == 0)
									arrayColor = DrawUtils::getWaveRGB(arrayMod->r1, arrayMod->g1, arrayMod->b1, arrayMod->r2, arrayMod->g2, arrayMod->b2, -index * (arrayMod->sped * -1));
							}
							if (arrayMod->rainBow.selected == 1) {
								if (arrayMod->rainBowDirection.selected == 1)
									arrayColor = DrawUtils::getRainbowWave(5, 1, 1, (-index * (arrayMod->sped * -1)) * 2);
								if (arrayMod->rainBowDirection.selected == 0)
									arrayColor = DrawUtils::getRainbowWave(5, 1, 1, (-index * arrayMod->sped) * 2);
							}
							...
							DrawUtils::fillRectangle(rectPos, MC_Color(12, 12, 12), ArrayList::opacity);
							if (!rainbowArray) {
							...
							if (!GameData::canUseMoveKeys() && rectPos.contains(&mousePos) && arrayMod) {
							...
							if (arrayMod->rainBow.selected == 0) {
								...
								for (char c : textStr) {
									...
									if (arrayMod->rainBowDirection.selected == 2) {
										MC_Color arrayColor = DrawUtils::getWaveRGB(arrayMod->r1, arrayMod->g1, arrayMod->b1, arrayMod->r2, arrayMod->g2, arrayMod->b2, -ind * arrayMod->sped);
										DrawUtils::drawText(textPos, &std::string(&c), arrayColor, textSize, 1);
										textPos.x += charWidth;
									} else if (arrayMod->rainBowDirection.selected == 3) {
										MC_Color arrayColor = DrawUtils::getWaveRGB(arrayMod->r1, arrayMod->g1, arrayMod->b1, arrayMod->r2, arrayMod->g2, arrayMod->b2, -ind * (arrayMod->sped * -1));
										DrawUtils::drawText(textPos, &std::string(&c), arrayColor, textSize, 1);
										textPos.x += charWidth;
									}
								}
								if (arrayMod->rainBowDirection.selected == 0 || arrayMod->rainBowDirection.selected == 1) {
									DrawUtils::drawText(textPos, &textStr, arrayColor);
								}

								// Modes
								DrawUtils::drawText(modePos, &textStr2, MC_Color(0.6f, 0.6f, 0.6f), textSize);
							}
							if (arrayMod->rainBow.selected == 1) {
								int ind = 1;
								for (char c : textStr) {
									...
									int charWidth = DrawUtils::getTextWidth(&std::string(&c), textSize);
									if (arrayMod->rainBowDirection.selected == 2) {
										MC_Color arrayColor = DrawUtils::getRainbowWave(5, 1.f, 1, (-ind * (arrayMod->sped * -1)) * 2);
										DrawUtils::drawText(textPos, &std::string(&c), arrayColor, textSize, 1);
										textPos.x += charWidth;
									} else if (arrayMod->rainBowDirection.selected == 3) {
										MC_Color arrayColor = DrawUtils::getRainbowWave(5, 1.f, 1, (-ind * arrayMod->sped) * 2);
										DrawUtils::drawText(textPos, &std::string(&c), arrayColor, textSize, 1);
										textPos.x += charWidth;
									}
								}
...
float* Hooks::Dimension_getFogColor(__int64 _this, float* color, __int64 a3, float a4) {
	...
	if (rainbowSkyMod->isEnabled()) {
		if (rainbowSkyMod->rainbow) {
			...
			color[0] = rainbowSkyMod->rFloat;
			color[1] = rainbowSkyMod->gFloat;
			color[2] = rainbowSkyMod->bFloat;
			return color;
		}
	}
...
void Hooks::chatLogHookFunc(__int64 a1, TextHolder* msg, uint32_t a2) {
	...
	static auto killInsults = moduleMgr->getModule<KillInsults>();
	...
	if (message.getText() != nullptr) {
		...
		// KillInsults
		std::string name;
		if (killInsults->isEnabled() && player != nullptr) {
			name = player->getNameTag()->getText();
			name = Utils::sanitize(name);

			if (textStr.find(name) != std::string::npos && textStr.find("killed") != std::string::npos) killInsults->killed = true;
			if (textStr.find(name) != std::string::npos && textStr.find("blown") != std::string::npos) killInsults->killed = true;
			if (textStr.find(name) != std::string::npos && textStr.find("shot") != std::string::npos) killInsults->killed = true;
			if (textStr.find(name) != std::string::npos && textStr.find("slain") != std::string::npos) killInsults->killed = true;
		}
	}
...
__int64 Hooks::ChestScreenController_tick(C_ChestScreenController* a1) {
	...
	if (AutoArmorMod->isEnabled() && AutoArmorMod->Mode.selected == 1) AutoArmorMod->craftingScreenController_tick((C_CraftingScreenController*)a1);

	static auto TotemMod = moduleMgr->getModule<AutoTotem>();
	if (TotemMod->isEnabled()) TotemMod->craftingScreenController_tick((C_CraftingScreenController*)a1);
...
bool Hooks::Mob__isImmobile(C_Entity* ent) {
	...
	static auto antiImmobileMod = moduleMgr->getModule<NoSlowDown>();
	if (antiImmobileMod->isEnabled() && antiImmobileMod->antii && ent == g_Data.getLocalPlayer())
		return false;
...
__int64 Hooks::inventoryScreen__tick(C_CraftingScreenController* a1) {
	...
	static auto AutoArmorMod = moduleMgr->getModule<AutoArmor>();
	if (AutoArmorMod->isEnabled() && AutoArmorMod->Mode.selected == 1) AutoArmorMod->craftingScreenController_tick(a1);

	static auto TotemMod = moduleMgr->getModule<AutoTotem>();
	if (TotemMod->isEnabled()) TotemMod->craftingScreenController_tick(a1);
	return oFunc(a1);
}
...
void Hooks::Actor__setRot(C_Entity* _this, vec2_t& angle) {
	...
	auto autoCrystalMod = moduleMgr->getModule<AutoCrystal>();

	// if (killauraMod->isEnabled() && g_Data.getLocalPlayer() == _this && !killauraMod->targetListA && killauraMod->rots.selected == 0)
	//	angle = {killauraMod->joe};
	if (freelookMod->isEnabled() && g_Data.getLocalPlayer() == _this)
		angle = freelookMod->oldPos;

	//if (autoCrystalMod->isEnabled() && autoCrystalMod->rotations.GetSelectedEntry().GetValue() == autoCrystalMod->rotations_normal && g_Data.getLocalPlayer() == _this && !autoCrystalMod->toBreak.empty())
		//angle = autoCrystalMod->breakRot;

	//if (autoCrystalMod->isEnabled() && autoCrystalMod->rotations.GetSelectedEntry().GetValue() == autoCrystalMod->rotations_normal && g_Data.getLocalPlayer() == _this && !autoCrystalMod->toPlace.empty())
		//angle = autoCrystalMod->placeRot;
...
void Hooks::onPOVChange(__int64 a1, unsigned int pov) {
	auto oFunc = g_Hooks.onPOVChangeHook->GetFastcall<void, __int64, unsigned int>();
