...
public:
	...
	void playStringSound(std::string sound, vec3_t const &position, float volume, float pitch) {
	...
	std::vector<C_Entity *> getMiscEntityList() {
		...
		if (func != 0) {
			...
			__int64 listSize = ((*reinterpret_cast<__int64 *>(reinterpret_cast<__int64>(listStart) + 0x8)) - (*listStart)) / 0x8;
