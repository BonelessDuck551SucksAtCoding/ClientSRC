...
C_MovePlayerPacket::C_MovePlayerPacket(C_LocalPlayer* player, vec3_t pos, bool onground) {
	static uintptr_t** movePlayerPacketVtable = 0x0;
	if (movePlayerPacketVtable == 0x0) {
		uintptr_t sigOffset = FindSignature("48 8D 05 ? ? ? ? 49 89 40 08 48 8B 47 30");
		int offset = *reinterpret_cast<int*>(sigOffset + 3);
...
ItemStackRequestPacket::ItemStackRequestPacket() {
	static uintptr_t** itemStackRequestPacketVtable = 0x0;
	if (itemStackRequestPacketVtable == 0x0) {
		uintptr_t sigOffset = FindSignature("48 8D 3D ? ? ? ? 48 89 7C 24 ? 48 89 4C 24");
		int offset = *reinterpret_cast<int*>(sigOffset + 3);
		itemStackRequestPacketVtable = reinterpret_cast<uintptr_t**>(sigOffset + offset + /*length of instruction*/ 7);
#ifdef _DEBUG
		if (itemStackRequestPacketVtable == 0x0 || sigOffset == 0x0)
			__debugbreak();
#endif
	}
	memset(this, 0, sizeof(ItemStackRequestPacket));  // Avoid overwriting vtable
	vTable = itemStackRequestPacketVtable;
}
