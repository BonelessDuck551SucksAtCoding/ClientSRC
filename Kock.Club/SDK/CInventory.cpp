...
void C_ContainerScreenController::handleAutoSwap(std::string name1, int too, std::string name2, int from) {
	using ContainerScreenController__swap = __int64(__fastcall*)(C_ContainerScreenController*, TextHolder, int, TextHolder, int);
	static ContainerScreenController__swap swapFunc = reinterpret_cast<ContainerScreenController__swap>(FindSignature("48 89 5c 24 ? 48 89 6c 24 ? 48 89 74 24 ? 57 41 54 41 55 41 56 41 57 48 81 ec ? ? ? ? 49 8b e9 45 8b f0"));
	if (swapFunc != 0x0)
		swapFunc(this, TextHolder(name1), too, TextHolder(name2), from);
}
